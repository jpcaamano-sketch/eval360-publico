...
# ============================================================
# ENRUTAMIENTO POR TOKEN (formulario público)
# ============================================================
...
if _token:
    ...
    _participante = queries.obtener_participante_por_token(_token)
    _evaluador = None if _participante else queries.obtener_evaluador_por_token(_token)

    if not _participante and not _evaluador:
    ...
    if _participante:
        ...
        if _participante["autoevaluacion_completada"]:
            st.success(f"¡Gracias, {_participante.get('nombre', '')}! Tu autoevaluación ya fue completada.")
            st.balloons()
            st.stop()

        _grupo = _participante.get("v2_grupos", {})
        _plantilla_id = _grupo.get("plantilla_id") if _grupo else None
        if not _plantilla_id:
            st.error("Error de configuración: no se encontró la plantilla asociada.")
            st.stop()

        _competencias = queries.listar_competencias_por_plantilla(_plantilla_id)
        if not _competencias:
            st.error("La plantilla no tiene competencias configuradas.")
            st.stop()

        st.title("Autoevaluación 360°")
        st.markdown(f"**Hola, {_participante.get('nombre', '')}**")
        ...
        _hc = st.columns([0.4, 4.5, 0.8, 0.8, 0.8, 1.0, 0.8])
        _hc[0].markdown("**#**"); _hc[1].markdown("**Competencia**")
        _hc[2].markdown("**Nunca**"); _hc[3].markdown("**Rara vez**")
        _hc[4].markdown("**A veces**"); _hc[5].markdown("**Frecuent.**"); _hc[6].markdown("**Siempre**")
        st.markdown("---")

        _respuestas = {}
        _todas_respondidas = True

        def _make_auto_cb(cid, val):
            def cb():
        ...
        for _idx, _comp in enumerate(list(_competencias), 1):
            _cid = _comp["id"]
            _rc = st.columns([0.4, 4.5, 0.8, 0.8, 0.8, 1.0, 0.8])
            with _rc[0]: st.markdown(f"**{_idx}**")
            with _rc[1]: st.markdown(_comp["texto_auto"])
            for _ci, _v in enumerate(range(1, 6)):
                with _rc[_ci + 2]:
                    st.checkbox(" ", key=f"auto_{_cid}_{_v}", on_change=_make_auto_cb(_cid, _v), label_visibility="hidden")
            _sel = next((v for v in range(1, 6) if st.session_state.get(f"auto_{_cid}_{v}")), None)
            if _sel is not None:
                _respuestas[_cid] = _sel
            else:
                _todas_respondidas = False
        ...
        for _i in range(5):
            ...
            with _c1:
                _nev = st.text_input("Nombre", key=f"ev_nombre_{_i}", label_visibility="hidden", placeholder=f"Nombre evaluador {_i+1}")
            with _c2:
                _eev = st.text_input("Email", key=f"ev_email_{_i}", label_visibility="hidden", placeholder=f"Email evaluador {_i+1}")
            if _nev.strip() and _eev.strip():
                _evaluadores_data.append({"nombre": _nev.strip(), "email": _eev.strip()})
        st.divider()
        ...
        if st.button("Guardar Autoevaluación", use_container_width=True, type="primary"):
            ...
            if _errores:
                ...
                try:
                    queries.guardar_respuestas_auto(_participante["id"], _respuestas)
                    queries.actualizar_participante(_participante["id"], {
                    ...
                    st.stop()
                for _ev in _evaluadores_data:
                    try:
                        _persona = queries.buscar_persona_por_correo(_ev["email"])
                        _pers_rut = _persona["pers_rut"] if _persona else None
                        queries.crear_evaluador(_participante["id"], _ev["nombre"], _ev["email"], _pers_rut)
                    except Exception:
                        pass
    ...
    if _evaluador:
        ...
        if _evaluador["completado"]:
            st.success(f"¡Gracias, {_evaluador['nombre']}! Tu evaluación ya fue completada.")
            st.balloons()
            st.stop()

        _part_info = _evaluador.get("v2_participantes", {})
        _part_nombre = _part_info.get("nombre", "el participante") if _part_info else "el participante"
        _part_id = _part_info.get("id") if _part_info else None
        _grupo_info = _part_info.get("v2_grupos", {}) if _part_info else {}
        _plantilla_id = _grupo_info.get("plantilla_id") if _grupo_info else None

        if not _plantilla_id or not _part_id:
        ...
        st.markdown(f"**Hola, {_evaluador['nombre']}** — Estás evaluando a **{_part_nombre}**. Tu feedback es **anónimo**.")
        ...
        for _idx, _comp in enumerate(list(_competencias), 1):
            ...
            _rc = st.columns([0.4, 4.5, 0.8, 0.8, 0.8, 1.0, 0.8])
            with _rc[0]: st.markdown(f"**{_idx}**")
            with _rc[1]: st.markdown(_comp["texto_feedback"])
            for _ci, _v in enumerate(range(1, 6)):
                with _rc[_ci + 2]:
                    st.checkbox(" ", key=f"fb_{_cid}_{_v}", on_change=_make_fb_cb(_cid, _v), label_visibility="hidden")
            _sel = next((v for v in range(1, 6) if st.session_state.get(f"fb_{_cid}_{v}")), None)
            if _sel is not None:
                _respuestas[_cid] = _sel
            else:
                _todas_respondidas = False
        ...
        if st.button("Guardar Feedback", use_container_width=True, type="primary"):
            if not _todas_respondidas or len(_respuestas) < len(_competencias):
                ...
                try:
                    queries.guardar_respuestas_feedback(_part_id, _evaluador["id"], _respuestas)
                except Exception as e:
                    st.error(f"Error al guardar las respuestas: {e}")
                    st.stop()
                try:
                    queries.actualizar_evaluador(_evaluador["id"], {
                        "completado": True,
...
def _contenido_importar_encuesta_csv():
    ...
    if st.button("🤖 Generar versiones con IA", use_container_width=True, type="primary"):
        ...
        prompt = (
            ...
            "     Ejemplo: 'Comunico con claridad mis ideas'\n"
            "  B) TERCERA PERSONA SINGULAR — solo cambia la conjugación verbal, SIN agregar sujeto\n"
            "     Ejemplo: 'Comunica con claridad sus ideas'\n\n"
            "Reglas estrictas:\n"
            "- Mantén el mismo significado y nivel de detalle\n"
            "- En la versión B: NO uses 'El/la evaluado/a', NO agregues ningún sujeto explícito\n"
            "- Solo conjuga los verbos en 3ª persona singular y cambia pronombres (mi→su, yo→él/ella si aplica)\n"
